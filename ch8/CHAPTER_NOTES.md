...
本書 第8章「監視と運用―― LLMアプリケーションの健全性管理」の各リストとリポジトリ実装の対応表です。
...
- **差分**:
  - 本書では `monitoring/04_evaluation.py` で「リアルタイムスコアラー」の設定例を示すと書かれていますが、リポジトリの実装は**蓄積されたトレースに対するバッチ評価**が中心です。
  - 真のリアルタイムスコアリング (本番リクエストごとの自動評価) は Databricks 環境での Production Monitoring 機能の利用が前提となります。OSS環境では本書のコードを参考に、Cron や定期ジョブで `make eval` を回す運用を想定しています。
